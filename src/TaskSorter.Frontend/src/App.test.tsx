...
const profile = {
  ...
  repositories: [{ id: '22222222-2222-2222-2222-222222222222', owner: 'owner', name: 'repo', fullName: 'owner/repo', repositoryTierId: '33333333-3333-3333-3333-333333333333', repositoryTierName: 'active', repositoryTierScore: 300, sortOrder: 0, positionScore: 2, priorityScore: 302, validation: [] }],
  labels: [
...
const tiers = [
  { id: '33333333-3333-3333-3333-333333333333', name: 'active', score: 300, isDefault: true, assignedRepositoryCount: 1, updatedAt: '2026-01-01T00:00:00Z' },
  { id: '44444444-4444-4444-4444-444444444444', name: 'paused', score: -200, isDefault: false, assignedRepositoryCount: 0, updatedAt: '2026-01-01T00:00:00Z' },
]
const rankedTask = {
...
function stubApi() {
  ...
  vi.stubGlobal('fetch', vi.fn(async (path: string, init?: RequestInit) => {
    ...
    if (path === `/api/profiles/${profile.id}` && !init?.method) return json(profile)
    if (path === `/api/profiles/${profile.id}` && init?.method === 'PUT') return json(profile)
    ...
    if (path === `/api/profiles/${profile.id}/labels/discover`) return json({
      labels: [
        profile.labels[0],
...
describe('App', () => {
  ...
  it('shows persisted repository rows with their selected tiers', async () => {
    stubApi()
    render(<App />)
    ...
    expect(screen.getByRole('button', { name: 'Drag owner/repo' })).not.toBeNull()
  })
